...
//			}
			...
			MenuService min = new MenuService();

			for (int i = 0; i < oinlist.length; i++) {
				oin = new OrderinvoiceVO();
				for (int x = 0; x < min.getAll().size(); x++) {
					if ((String.valueOf(oinlist[i].charAt(0))).equals((min.getAll().get(x)).getMenu_Id())) {
			...
			req.getSession().setMaxInactiveInterval(43200);
			req.getSession().setAttribute(iord, inv.size());
...
//			String[] tels ={tel};//上線用
			...
			if (ordertype == 1) {
				...
				message = "\t取餐資訊\n" + "訂單編號:" + req.getSession().getAttribute("ordNo") + "\n分店地址:"
						+ req.getSession().getAttribute("braAdr") + "\n取餐時間:" + req.getSession().getAttribute("time");
			} else if (ordertype == 2) {
				// 外送
				message = "\t取餐資訊\n" + "訂單編號:" + req.getSession().getAttribute("ordNo") + "\n取餐地址:"
						+ req.getSession().getAttribute("deliv_addres") + "\n取餐時間:"
						+ req.getSession().getAttribute("time");
			}
		...
		if ("getOne_For_Display_By_Mem_No".equals(action)) { // 來自select_page.jsp的請求
			...
			try {
				...
				RequestDispatcher successView = req
						.getRequestDispatcher("/front_end/orderform/listOneOrderformByMemNo.jsp");
				successView.forward(req, res);
