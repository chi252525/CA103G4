...
//			successView.forward(req, res);
			...
			req.getSession().setAttribute("shoppingcart", inv);// 取得送來的餐點參數
			req.getSession().setAttribute("shoppingcartCustom", customv);// 取得送來的自訂餐點參數
...
//			successView.forward(req, res);
		...
		if ("listEmps_ByCompositeQuery".equals(action)) { // 來自select_page.jsp的複合查詢請求
			...
			try {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/back_end/orderform/orderform.jsp");
				failureView.forward(req, res);
			}
		}
