	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("insert".equals(action)) {// 來自checkout.jsp的請求
			...
			try {
				...
				if (eatIn_takeAway == null) {
					errorMsgs.put("eatIn_takeAway", "請選擇取餐方式");
				}

				String county = req.getParameter("mem_Recounty");//取得鄉鎮市區
				String town = req.getParameter("mem_Retown");
				System.out.println("county,town="+county+town);
				if ("請選擇".equals(county)||"請選擇".equals(town)) {
					errorMsgs.put("countytwon", "請選擇鄉鎮市區");
				}
				
				String address = req.getParameter("deliv_addres");// 取得地址
				String addressRegx = "^[(\u4e00-\u9fa5)(a-zA-Z0-9_)]$";

				if (address == null || address.trim().length() == 0) {
					errorMsgs.put("deliv_addres", "外送地址 : 請勿空白");
				} else if (!address.trim().matches(addressRegx)) {
					errorMsgs.put("deliv_addres", "地址: 只能是中、英文字母、數字");
				}

				String branch_No = req.getParameter("branch_no"); // 取得分店編號
				if (branch_No.equals("請選擇")) {
					errorMsgs.put("branch_no", "請選擇分店");
				}
				...
				System.out.println("order_pstatus="+order_pstatus);
				if ("2".equals(order_pstatus)) {	//信用卡錯誤驗證
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req.getRequestDispatcher("/front_end/shoppingCart/Checkout.jsp");
				...
				/****************************
				 * 3.Send data to orderForm servlet
				 ***********/
				...
				req.getRequestDispatcher("/front_end/shoppingCart/Checkout.jsp").forward(req, res);
				return;
