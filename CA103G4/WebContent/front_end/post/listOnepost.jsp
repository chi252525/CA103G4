	...
	<div class="container container-margin">
		...
		<span class="badge badge-pill badge-info" style="font-size:18px;"><b>${ingredientsVO.ingdt_Name}</b></span>&nbsp;
				...
				<c:forEach var="replyVO" items="${rplyList}">
					<div class="media my-4 px-2 py-2 "
						style="background-color: rgba(255, 255, 255, 0.45)">
						<img class="d-flex mr-3 rounded-circle"
							src="<%=request.getContextPath()%>/front_end/member/member.do?mem_No=${replyVO.mem_No}"
							style="display:${(memVO.mem_Name == null )? 'none': ''};height:50px;width:50px;border-radius:50%;">
						<div class="my-0 ">
							...
							<div class="btn-group">
								...
								<a
									href="<%=request.getContextPath()%>/reply/replyServlet.do?action=delete&rply_No=${replyVO.rply_No}"
									class="btn btn-default btn-sm ${(memVO.mem_No==replyVO.mem_No)?'':'disabled'}"
									onclick="deleteConfirm()">刪除</a>
			...
			<div class="modal-content">
				...
				<FORM METHOD="post"
					...
					<div class="modal-footer">
						...
						<input type="hidden" name="mem_No" value="${memVO.mem_No}" /> <input
							type="hidden" name="post_No" value="${postVO.post_No}" />
						<button type="submit" class="btn btn-primary" name="action"
							value="insert">同意並送出</button>
					</div>
				</form>
			</div>
